...
def min_max_scale(img, dtype = None):
    np.seterr(all = 'raise')
...
def stack_gui(tiff_list, norm_list):
    ...
    def getAlphaBeta(img, min_val, max_val):
        min_percent = round(percentileofscore(img.flatten(), min_val, kind = 'weak'))
        max_percent = round(percentileofscore(img.flatten(), max_val, kind = 'weak'))
